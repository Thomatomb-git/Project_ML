...
import joblib  # Digunakan untuk load model SVR (.pkl)
...
def main():
    print("=" * 70)
    print("  CEK PERSENTASE ERROR MODEL PREDIKSI HARGA IHSG (SVR MODE)")
    print("=" * 70)
    
    # 1. Memuat data
    X_train, y_train, X_val, y_val, X_test, y_test, scalery = load_and_split_data_scaled()
    
    # 2. Memuat model-model SVR yang sudah dilatih (.pkl)
    models = {}
    model_files = {
        "SVR + Optuna": os.path.join("outputs", "models", "svr_optuna.pkl"),
        "SVR + GridSearch": os.path.join("outputs", "models", "svr_gridsearch.pkl"),
    }
    
    for name, path in model_files.items():
        if os.path.exists(path):
            # Memuat model SVR menggunakan joblib secara murni
    ...
    if not models:
        print("Error: Tidak ada model SVR yang ditemukan! Jalankan run_optuna.py / run_gridsearch.py dulu.")
        return
    
    # 3. Hitung metrik persentase error untuk setiap model SVR
